    ...
    <el-row>
        <el-form inline>
            ...
            <el-form-item>
              ...
              <el-button type="primary" @click="handleFocus">焦点图</el-button>
            </el-form-item>
        </el-form>
    </el-row>
    <el-row>
...
<script>
import { getProduce, delProduce, sortProduce, recoverProduce} from "apis/";
...
export default {
  ...
  methods: {
      ...
      handleFocus() {
        this.$router.push({name:'focus',query: {'type': this.item.id, 'name': this.item.menuName}});
      },
    getList() {
    ...
    handleCommand(command, record) {
        ...
        if(command == 1) {
            // 编辑
            this.$router.push({name: 'produceNew',query: {id: record.id, type: record.columntype}})
        } else if (command == 2) {
             // 上下架
            if(record.status === 1) {
                this.handleDel();
    ...
    handleRecover() {
        ...
        recoverProduce(params).then(res => {
            this.$message({
              type: "success",
              message: "恢复成功"
            });
    ...
    handleNew(type) {
      this.$router.push({name: 'produceNew',query: {type: this.item.id, name: this.item.menuName}});
    }
  }
};
</script>
