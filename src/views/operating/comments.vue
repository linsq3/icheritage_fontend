...
<script>
import { getComments, commentsAgree, commentsRefused } from "apis/";
      ...
      columns: [
        {
          ...
          render: (record, index, h) => {
            return (
                <div>
                <div style="float:left;margin-right: 10px;">评价图片</div>
                {record.url.map(v=>
                        <img class="comment-img" src={basePath + v.url} />                   
                )}
                </div>
            );
          }
  ...
  methods: {
    getList() {
      ...
      getComments(params).then(res => {
        this.tableData = res.data.commentlists;
    ...
    handleRefused(id) {
        ...
        commentsRefused(params).then(res => {
            this.$message({
                type: "success",
                message: "审核不通过"
            });
